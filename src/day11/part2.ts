...
const totalOctopuses: number = energiesMatrix.length * energiesMatrix[0].length;
...
for (let step = 1; step <= steps; step++) {
  ...
  if (flashesPerStep[step] === totalOctopuses) {
    console.log('🚀 ~ file: index.ts ~ line 79 ~ step', step);
    break;
  }
}
